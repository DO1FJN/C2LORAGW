 ...
 *
 * Report:
 * 2012-02-09 JA CRC-CCITT is endian independent, using "compiler.h"
 */
...
const unsigned short cTabCCITT[256] = {
...
const unsigned short tab_ccitt_revers[256] = {
...
#define _crc32(accum, delta)	(accum)=cTabCRC32[((accum)^(delta))&0xff]^((accum)>>8)


#define _crcCCITT(crc, data)	(((U16)(LSB(crc) << 8) ^ cTabCCITT[MSB(crc) ^ (U8)(data)]))
...
*  and saves it in CRC                  *
****************************************/
__inline void crcCCITT(unsigned short *crc, char data) {      //CRC for one byte
  *crc = _crcCCITT(*crc, data);
}


__inline void crc32(unsigned long *crc, const char data) {
  _crc32(*crc, data);
}
...
/****************************************
*  calculates the checksum of the data  *
*  pointed to data with length bytes    *
*  and returns it in CRC                *
****************************************/
unsigned short crcCCITTBlock(const char *data, int length) {
  unsigned short crc = 0;
  for (; length>0; length--) {                             // calculate the CRC
    crc = _crcCCITT(crc, *data);
    data++;
  } // for
  return crc;
...
*  and returns it in CRC                *
****************************************/
__inline void crcCCITTStream(unsigned short *crc, const char *data, int length) {
  for (; length>0; length--) {                             // calculate the CRC
    *crc = _crcCCITT(*crc, *data);
    data++;
  } // for
...
__inline void crc32Stream(unsigned long *crc, const char *data, int length) {
  for (; length>0; length--) {                             // calculate the CRC
    _crc32(*crc, *data);
    data++;
  } // for
...
void append_crc_ccitt_revers(char *buffer, unsigned int len) {
  ...
  crc = 0xFFFF;
  for (; len>2; len--) {
    crc = MSB(crc) ^ tab_ccitt_revers[LSB(crc)^(*buffer)];
    buffer++;
  }
  crc       = ~crc;
  *buffer++ = LSB(crc);
